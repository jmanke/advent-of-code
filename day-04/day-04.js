...
// I'll let you borrow my boat and you can go visit the gardener. I got all these scratchcards as a gift, but I can't
...
// already scratched off. Picking one up, it looks like each card has two lists of numbers separated by a vertical bar (|): a
// list of winning numbers and then a list of numbers you have. You organize the information into a table (your puzzle input).
...
function partOne() {
  ...
  return input.reduce((sum, card) => {
    const [_, g1, g2] = card.match(/.*: (.*)\|(.*)/);
    const winning = new Set(toNumbers(g1));
    const numbers = toNumbers(g2);

    const points = numbers.reduce((points, number) => {
      const isWinningNumber = winning.has(number);
...
// Just as you're about to report your findings to the Elf, one of you realizes that the rules have actually been printed on the
// back of every card this whole time.
...
//  8 instances of card 4, 14 instances of card 5, and 1 instance of card 6. In total, this example pile of scratchcards causes you to
...
function partTwo() {
  ...
  input.forEach((card, i) => {
    ...
    const wins = numbers.reduce((points, number) => {
      const isWinningNumber = winning.has(number);
